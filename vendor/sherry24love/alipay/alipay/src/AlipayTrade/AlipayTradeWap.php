<?php
namespace EasyAlipay\AlipayTrade ;

use EasyAlipay\Core\AopClient;
use EasyAlipay\Core\Request\AlipayTradeWapPayRequest ;
use Exception ;
use Log ;
use EasyAlipay\AlipayTrade\ContentBuilder\AlipayTradeWapPayContentBuilder;

class AlipayTradeWap {
	...
	function __construct( $config ){
		if( data_get( $config , 'gatewayUrl' ) ) {
			$this->gateway_url = data_get( $config , 'gatewayUrl' ) ;
		}
		
		$this->appid = data_get( $config , 'app_id' );
		$this->private_key = data_get( $config , 'merchant_private_key' ) ;
		$this->alipay_public_key = data_get( $config , 'alipay_public_key' );
		$this->charset = data_get( $config , 'charset' );
		$this->signtype= data_get( $config , 'sign_type' );
	 ...
	 *  $builder = new AlipayTradeWapPayContentBuilder();
		...
		$builder->setTotalAmount($total_amount);
		$builder->setTimeExpress($timeout_express);
	 * @param $return_url 同步跳转地址，公网可访问
	 * @param $notify_url 异步通知地址，公网可以访问
	 * @return $response 支付宝返回的信息
	 */
	function pay( $builder , $return_url = null , $notify_url =null ) {
